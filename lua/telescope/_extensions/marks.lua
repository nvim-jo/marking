...
local marking_mark = require("marking.mark")
...
local generate_new_finder = function()
    return finders.new_table({
        results = prepare_results(marking.get_mark_config().marks),
...
local delete_marking_mark = function(prompt_bufnr)
    ...
    marking_mark.rm_file(selection.filename)
    ...
    for _, current_selection in ipairs(selections) do
        marking_mark.rm_file(current_selection.filename)
    end
...
local move_mark_up = function(prompt_bufnr)
    local selection = action_state.get_selected_entry()
    local length = marking_mark.get_length()

    if selection.index == length then
        return
    end

    local mark_list = marking.get_mark_config().marks
...
return function(opts)
    ...
    pickers.new(opts, {
        prompt_title = "marking marks",
        finder = generate_new_finder(),
        sorter = conf.generic_sorter(opts),
        previewer = conf.grep_previewer(opts),
        attach_mappings = function(_, map)
            map("i", "<c-d>", delete_marking_mark)
            map("n", "<c-d>", delete_marking_mark)
