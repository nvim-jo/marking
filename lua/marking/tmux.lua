...
if global_config.tmux_autoclose_windows then
    local marking_tmux_group = vim.api.nvim_create_augroup(
        "MARKING_TMUX",
        { clear = true }
    )
    ...
    vim.api.nvim_create_autocmd("VimLeave", {
        callback = function()
            require("marking.tmux").clear_all()
        end,
        group = marking_tmux_group,
    })
end
...
function M.sendCommand(idx, cmd, ...)
    ...
    if type(cmd) == "number" then
        cmd = marking.get_term_config().cmds[cmd]
    end
...
function M.get_length()
    ...
    return table.maxn(marking.get_term_config().cmds)
end
...
function M.emit_changed()
    ...
    if marking.get_global_settings().save_on_change then
        marking.save()
    end
end
...
function M.add_cmd(cmd)
    ...
    marking.get_term_config().cmds[found_idx] = cmd
    M.emit_changed()
end
...
function M.rm_cmd(idx)
    ...
    table.remove(marking.get_term_config().cmds, idx)
    M.emit_changed()
end

function M.set_cmd_list(new_list)
    log.trace("set_cmd_list(): New list:", new_list)
    for k in pairs(marking.get_term_config().cmds) do
        marking.get_term_config().cmds[k] = nil
    end
    for k, v in pairs(new_list) do
        marking.get_term_config().cmds[k] = v
    end
    M.emit_changed()
end
