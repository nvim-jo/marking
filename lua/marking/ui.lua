...
local Marked = require("marking.mark")
...
local function close_menu(force_save)
    ...
    if global_config.save_on_toggle or force_save then
        require("marking.ui").on_menu_save()
    end

    vim.api.nvim_win_close(Marking_win_id, true)
...
local function create_window()
    ...
    local Marking_win_id, win = popup.create(bufnr, {
        title = "󰐾 Marking",
    ...
    return {
        bufnr = bufnr,
        win_id = Marking_win_id,
    }
end

local function get_menu_items()
    log.trace("_get_menu_items()")
    local lines = vim.api.nvim_buf_get_lines(Marking_bufh, 0, -1, true)
    local indices = {}
...
function M.toggle_quick_menu()
    ...
    if Marking_win_id ~= nil and vim.api.nvim_win_is_valid(Marking_win_id) then
        close_menu()
        return
    end
    ...
    Marking_win_id = win_info.win_id
    Marking_bufh = win_info.bufnr
    ...
    vim.api.nvim_win_set_option(Marking_win_id, "number", true)
    vim.api.nvim_buf_set_name(Marking_bufh, "marking-menu")
    vim.api.nvim_buf_set_lines(Marking_bufh, 0, #contents, false, contents)
    vim.api.nvim_buf_set_option(Marking_bufh, "filetype", "marking")
    vim.api.nvim_buf_set_option(Marking_bufh, "buftype", "acwrite")
    vim.api.nvim_buf_set_option(Marking_bufh, "bufhidden", "delete")
    vim.api.nvim_buf_set_keymap(
        Marking_bufh,
        "n",
        "q",
        "<Cmd>lua require('marking.ui').toggle_quick_menu()<CR>",
        { silent = true }
    )
    vim.api.nvim_buf_set_keymap(
        Marking_bufh,
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "<CR>",
        "<Cmd>lua require('marking.ui').select_menu_item()<CR>",
        {}
    )
    vim.cmd(
        string.format(
            "autocmd BufWriteCmd <buffer=%s> lua require('marking.ui').on_menu_save()",
            Marking_bufh
        )
    )
    if global_config.save_on_change then
        vim.cmd(
            string.format(
                "autocmd TextChanged,TextChangedI <buffer=%s> lua require('marking.ui').on_menu_save()",
                Marking_bufh
            )
        )
    end
    ...
    vim.cmd(
        "autocmd BufLeave <buffer> ++nested ++once silent lua require('marking.ui').toggle_quick_menu()"
    )
end
