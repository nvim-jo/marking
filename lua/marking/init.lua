...
local user_config = string.format("%s/marking.json", config_path)
local cache_config = string.format("%s/marking.json", data_path)

local M = {}

local nvim_jo_marking = vim.api.nvim_create_augroup(
    "NVIM_JO_MARKING",
    { clear = true }
)
...
vim.api.nvim_create_autocmd({ "BufLeave, VimLeave" }, {
    callback = function()
        require("marking.mark").store_offset()
    end,
    group = nvim_jo_marking,
})
...
vim.api.nvim_create_autocmd("FileType", {
    ...
    callback = function()
        -- Open marking file choice in useful ways
        --
...
--]]
MarkingConfig = MarkingConfig or {}
...
function M.save()
    ...
    Path:new(cache_config):write(vim.fn.json_encode(MarkingConfig), "w")
end
...
function M.setup(config)
    ...
    if complete_config.tabline then
        require("marking.tabline").setup(complete_config)
    end

    MarkingConfig = complete_config

    log.debug("setup(): Complete config", MarkingConfig)
    log.trace("setup(): log_key", Dev.get_log_key())
end

function M.get_global_settings()
    log.trace("get_global_settings()")
    return MarkingConfig.global_settings
end
...
function M.refresh_projects_b4update()
    ...
    local current_p_config = {
        projects = {
            [cwd] = ensure_correct_config(MarkingConfig).projects[cwd],
        },
    }

    -- erase all projects from global config, will be loaded back from disk
    MarkingConfig.projects = nil
    ...
    -- don't override non-project config in MarkingConfig later
    ...
    MarkingConfig = complete_config
    log.debug("refresh_projects_b4update(): Complete config", MarkingConfig)
    log.trace("refresh_projects_b4update(): log_key", Dev.get_log_key())
end

function M.get_term_config()
    log.trace("get_term_config()")
    return ensure_correct_config(MarkingConfig).projects[utils.project_key()].term
end

function M.get_mark_config()
    log.trace("get_mark_config()")
    return ensure_correct_config(MarkingConfig).projects[mark_config_key()].mark
end

function M.get_menu_config()
    log.trace("get_menu_config()")
    return MarkingConfig.menu or {}
end

-- should only be called for debug purposes
function M.print_config()
    print(vim.inspect(MarkingConfig))
end
